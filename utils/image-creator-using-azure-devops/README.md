...
- `azure-pipelines.yml`: The Installer will be called with option `create container-image`. The entrypoint will be defined and set by Installer dependent on selected product. It can be only installed products which are ready for running inside container.
- `azure-pipelines-own-entrypoint.yaml`: This is so called *lagacy* version. A Dockerfile is created and after this, `docker build` will be called to create the image. During the build process, Installer will be called to install the products. The products will be installed in the old legacy fashion and you can install all available components. This way offers a parameter `entrypoint`. If the container is creating and starting, the entrypoint command is executed also.
...
* `Variable Group`: Use this parameter to parameterize the running pipeline with a set of variables grouped in a Variable Group. The default Variable Group is `webmethods-image-creator-vars`.
...
The running pipeline needs additional parameters which are passed as Variable Group `webmethods-image-creator-vars` to the job. Create a Variable Group in the Azure project with following variables:
...
* `AGENT_POOL`: Default is `wm-image-creator`. The running pipeline use the Agent Pool to address the registered agent for executing the pipeline.

**Tip:** If you plan to have multiple environments to create images, add the environment name to Variable Group name. The Variable Group name is requested on starting the pipeline.
...
My webMethods Server with base image `centos:7|8`:
...
*The access from running container inside Kubernetes to Docker (DtoD,D2D) can be restricted in newer Kubernetes version. Therefore, check your Kubernetes distribution and maybe it is easier to run only Docker in your DevOps environment to build images.*

To start and run an Azure Pipeline, you need an Azure project. The pipelines must be assigned and for remote executing an Agent Pool e.g. `wm-image-creator`. An Azure project already created on <https://dev.azure.com/wM-Inno-Container/webmethods-image-creator>.

The pipeline job can be executed in Kubernetes. This requires an running agent. The agent must know the Azure project. To do this, an agent pool `wm-image-creator` must exist. 
...
helm upgrade --install azure-pipelines-agent emberstack/azure-pipelines-agent ^
 ...
 --set pipelines.pool=wm-image-creator
```
...
**Note:** `wm-image-creator` must be set in variable `AGENT_POOL` of Variable Group.

**Note:** `<Your-Personal-Access-Token>` create the token in your profile

## Configure Azure Project for Agent Pool 

The Azure project must be configured regarding Agent Pool `wm-image-creator`. Go to your Project Settings > Pipelines > Agent pools and add a new pool called `wm-image-creator` with Pool type `Self-hosted`. After a short time, you should find an agent in menu `Agents` with the name which you have defined as release on command `helm upgrade --install <release-name> ...`.

**Tip:** If you have different locations or environments to create images, add the environment into the Agent Pool name. Also, you must set this name into `AGENT_POOL` variable.

## Inside Pipeline Job

To support Kubernetes stateless Pod environment, the pipeline job does following steps to create the image:
...
6. Push created image to remote registry.
7. Remove created image in local registry.
